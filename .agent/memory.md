...
<!-- END:180 -->
<!-- ID:181 -->
PS exec脚本判例: PowerShell整脚本先解析后执行, 单一语法错(本轮事故=@ @(双@手滑))=全脚本零执行, 回执仅ParserError无任何段落输出; 长取证脚本要么分段指令隔离爆炸半径, 要么发出前自查配对; 判读ParserError时结论永远是"零副作用纯重跑", 不要误读为部分执行
tag: 脚本判例, PowerShell, exec, 全灭重跑
<!-- END:181 -->
<!-- ID:182 -->
刀Z首刀断言判例: 内容锚定手术的锚必须全局唯一+计数强制——PE ‘共存安全网’ 出现2次(类头8+catch注释213, 后者在从未读过的PE100-223区段), 断言len==1拦刀, 桌审(基于已读区段的模型)无法发现未读区段的重复锚; 修正法=多条件锚(如+独特限定词’手术单’双条件), touch必须断言count==1而非仅打印; 尸检B段(全文件子串计数+行号表)是暴露未建模触点/重复锚的标准手段, 应作为动刀前常规而非失败后补救
tag: 脚本判例, 断言网, 内容锚定, 刀Z, 尸检
<!-- END:182 -->
<!-- ID:183 -->
刀Z判例·触点搜索范围: "全库搜索"若只单包递归=画错圈——handcode包外还有core包(tannyjung.tanshugetrees_core), EventCenter.java:236的TreeLocation.start调用在core包, 三轮取证全漏(搜索边界=handcode递归), 物理删除后编译器击毙悬挂引用才暴露; 铁律: 删除类成员前, 触点搜索必须覆盖整棵src树(两包全量), 运行验收不能替代编译验证(让路门短路让悬挂在运行时零症状); 防线序列: 断言网拦逻辑错(重复锚), 编译器拦结构错(悬挂引用), 各拦一刀
tag: 刀Z, 断言网, 编译验证, 触点搜索, 搜索边界, 教训
<!-- END:183 -->
<!-- ID:184 -->
